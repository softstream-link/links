        ...
        fn links_bindings_python(_py: Python, m: &PyModule) -> PyResult<()> {
                ...
                use links_nonblocking::{
                    prelude::{
                        ...
                        *},
                    unittest::setup::{
                        connection::{CltTest, CltTestSender, SvcTest, SvcTestSender, CltTestSenderRef, SvcTestSenderRef},
                        protocol::{CltTestProtocolManual, SvcTestProtocolManual, CltTestProtocolAuthAndHbeat, SvcTestProtocolAuthAndHBeat},
                ...
                impl SvcConfig {
                    ...
                    fn default_auto() -> Self {
                        Self { max_connections: NonZeroUsize::new(1).unwrap(), io_timeout: Some(0.5), name: Some(asserted_short_name!("SvcAuto", SvcAuto).to_owned()) }
                    }
                ...
                impl CltConfig {
                    ...
                    fn default_auto() -> Self {
                        Self { connect_timeout: 1., retry_connect_after: 0.1, io_timeout: Some(0.5), name: Some(asserted_short_name!("CltAuto", CltAuto).to_owned()) }
                    }
                    fn from_auto(kwargs: &PyDict) -> Self {
                        let default = Self::default_auto();
                        let connect_timeout = kwargs.get_item("connect_timeout").unwrap().map_or(default.connect_timeout, |any| any.extract::<f64>().unwrap());
