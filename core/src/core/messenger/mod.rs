...
pub trait Messenger: Framer+Debug+Send+Sync+'static { // TODO Once Arc<C> is removed from signature see if removing Synch is possible, Send is still likely required because we should be able to pass CltRecver to a Poll Thread
