...
impl<T: Debug> CanonicalEntry<T> {
    pub fn try_into_recv(self) -> T {
        match self.msg {
            Message::Recv(t) => t,
            Message::Sent(_) => panic!(
                "{}::try_into_recv: Not {}::Recv variant",
...
impl<T: Debug+Send+Sync+Clone> CanonicalEntryStore<T> {
    ...
    pub fn find<P: FnMut(&CanonicalEntry<T>) -> bool>(
        &self,
        con_id_name: &str,
        mut predicate: P,
        timeout: Option<Duration>,
    ) -> Option<CanonicalEntry<T>> {
        let now = Instant::now();
    ...
    pub fn find_sent<P: FnMut(&T) -> bool>(
        &self,
        con_id_name: &str,
        mut predicate: P,
        timeout: Option<Duration>,
...
impl<T: Debug+Send+Sync+Clone> Display for CanonicalEntryStore<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(
            f,
            "{}<{}, {}>",
            asserted_short_name!("CanonicalEntryStore", Self),
            short_type_name::<T>(),
            msgs.len()
        )?;
...
mod test {
    ...
    fn test_callback() {
        ...
        assert_eq!(
            last_svc.msg,
            Message::Sent(TestMsg::Svc(svc_on_sent_msg.clone()))
        );
        ...
        assert_eq!(
            last_clt.msg,
            Message::Sent(TestMsg::Clt(clt_on_send_msg.clone()))
        );
