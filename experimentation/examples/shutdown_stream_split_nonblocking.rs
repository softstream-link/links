...
fn clt2svc() -> Result<(), Error> {
    ...
    let clt = TcpStream::connect(addr)?;
    let (svc, _addr) = acp.accept()?;

    clt.set_nonblocking(true)?;
    let (mut clt_read, mut _clt_write) = (clt.try_clone()?, clt);
    svc.set_nonblocking(true)?;
    let (mut _svc_read, mut svc_write) = (svc.try_clone()?, svc);

    println!("clt_read: {:?}, clt_write: {:?}", clt_read, _clt_write);
    println!("svc_read: {:?}, svc_write: {:?}", _svc_read, svc_write);
    ...
    loop {
        match write_busywait(&mut svc_write) {
            Ok(n) => {
                if n == EOF {
