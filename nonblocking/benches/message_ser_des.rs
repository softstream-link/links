    ...
    c.bench_function(id.as_str(), |b| {
        b.iter(|| {
            black_box({
                ...
                let _x = TestCltMessenger::serialize::<TEST_MSG_FRAME_SIZE>(&msg).unwrap();
            })
        })
    });
}
...
fn deserialize_msg(c: &mut Criterion) {
    ...
    let msg = TestCltMsg::Dbg(TestCltMsgDebug::new(b"Hello Frm Client Msg"));
    let (buf, len) = TestCltMessenger::serialize::<TEST_MSG_FRAME_SIZE>(&msg).unwrap();
    let buf = &buf[..len];
    let id = format!("deserialize TestCltMsg");
    c.bench_function(id.as_str(), |b| {
        b.iter(|| {
            black_box({
                let _x = TestSvcMessenger::deserialize(buf).unwrap();
            })
        })
    });
}
