...
impl<T: Debug+Clone+Send+Sync+'static> EventStoreSync<T> {
    ...
    pub fn find<P: FnMut(&EntryOld<T>) -> bool>(&self, con_id_name: &str, predicate: P, timeout: Option<Duration>) -> Option<EntryOld<T>> {
        self.runtime.block_on(self.store.find(con_id_name, predicate, timeout))
    }
    pub fn find_recv<P: FnMut(&T) -> bool>(&self, con_id_name: &str, predicate: P, timeout: Option<Duration>) -> Option<T> {
        self.runtime.block_on(self.store.find_recv(con_id_name, predicate, timeout))
    }
    pub fn find_send<P: FnMut(&T) -> bool>(&self, con_id_name: &str, predicate: P, timeout: Option<Duration>) -> Option<T> {
        self.runtime.block_on(self.store.find_send(con_id_name, predicate, timeout))
    }
...
impl<T: Debug+Clone+Send+Sync+'static> EventStoreAsync<T> {
    ...
    pub async fn find<P: FnMut(&EntryOld<T>) -> bool>(&self, con_id_name: &str, mut predicate: P, timeout: Option<Duration>) -> Option<EntryOld<T>> {
        let now = Instant::now();
    ...
    pub async fn find_recv<P: FnMut(&T) -> bool>(&self, con_id_name: &str, mut predicate: P, timeout: Option<Duration>) -> Option<T> {
        let entry = self
            .find(
                con_id_name,
            ...
            .await;
        match entry {
            Some(EntryOld { event: DirOld::Recv(t), .. }) => Some(t),
            _ => None,
        }
    }
    pub async fn find_send<P: FnMut(&T) -> bool>(&self, con_id_name: &str, mut predicate: P, timeout: Option<Duration>) -> Option<T> {
        let entry = self
            .find(
                con_id_name,
                |entry| match entry.event {
            ...
            .await;
        match entry {
            Some(EntryOld { event: DirOld::Send(t), .. }) => Some(t),
            _ => None,
        }
    }
    pub fn last(&self) -> Option<EntryOld<T>> {
...
impl<T: Debug+Clone+Send+Sync+'static> Display for EventStoreAsync<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        fn writeln<T: Debug>(f: &mut std::fmt::Formatter<'_>, count: usize, delta_window: Duration, entry: &EntryOld<T>) -> std::fmt::Result {
...
mod test {
    ...
    fn test_callback() {
        ...
        let clt_clb = EventStoreCallback::<TestMsg, TestCltMsgProtocol>::new(event_store.async_ref());
        let svc_clb = EventStoreCallback::<TestMsg, TestSvcMsgProtocol>::new(event_store.async_ref());
        ...
        svc_clb.on_recv(&ConId::svc(Some("svc"), "0.0.0.0:0", None), svc_on_recv_msg.clone());
        svc_clb.on_send(&ConId::svc(Some("svc"), "0.0.0.0:0", None), &svc_on_send_msg);
        ...
        clt_clb.on_recv(&ConId::clt(Some("clt"), None, "0.0.0.0:0"), clt_on_recv_msg.clone());
        clt_clb.on_send(&ConId::clt(Some("clt"), None, "0.0.0.0:0"), &clt_on_send_msg);
        ...
        assert_eq!(last_svc.event, DirOld::Send(TestMsg::Svc(svc_on_send_msg.clone())));

        let last_clt = event_store.find("clt", |_| true, None).unwrap();
        info!("last_clt: {:?}", last_clt);
        assert_eq!(last_clt.event, DirOld::Send(TestMsg::Clt(clt_on_send_msg.clone())));
