...
pub type EventStoreAsyncRef<T> = Arc<EventStore<T>>;

#[derive(Debug)]
pub struct EventStore<T: Debug+Clone+Send+Sync+'static> {
    store: Mutex<Vec<EntryOld<T>>>,
}
impl<T: Debug+Clone+Send+Sync+'static> EventStore<T> {
...
impl<T: Debug+Clone+Send+Sync+'static> Default for EventStore<T> {
    fn default() -> Self {
        Self { store: Default::default() }
    }
}
impl<T: Debug+Clone+Send+Sync+'static> Display for EventStore<T> {
